import { Res, Req } from "../../frameworks/types/ServerTypes";
...
export class AdminController {
  ...
  static async updateUnverifiedAdminController(
    req: Req,
    res: Res
  ): Promise<void> {
    try {
      ...
      const data = await updatedAdmin.UnverifiedUserUpdate(
        req.body.firebaseConfirm,
        req.body.phone
      );
  ...
  static async loginVerifyAdminController(req: Req, res: Res): Promise<void> {
    try {
      ...
      const data = await loggedIn.LoginVerifyAdmin(
        req.body.email,
        req.body.password
      );
      ...
      if (data !== 'Invalid credentials') {
        ...
        res.json({ loggedIn: false , message: data});
      }
    } catch (error) {
      console.log(error);
    }
  ...
  static async UpdateAdminProfileController(req: Req, res: Res): Promise<void> {
    try {
  ...
  static async GetAdminData(req: Req, res: Res): Promise<void> {
    const retrieveAdminData = new AdminUseCase(adminRepository);
    const data = await adminRepository.FindAdminById(req.params.id);
    res.json(data)
  }
}
