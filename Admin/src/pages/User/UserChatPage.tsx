...
const UserChatPage: React.FC = () => {
  ...
  }, [conId, currentUser.username, navigate]);
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post<Message>(
        "/user/user_send_message_to_admin",
        {
  ...
  const handleAudioUpload = async (
    ...
    try {
      ...
      const fileName =
        new Date().getTime() +
        "_" +
        (audioFile instanceof File ? audioFile.name : "audio.webm");
      const storageRef = ref(storage, fileName);
                    ...
                    {message.type === "text" ? (
                      ...
                      <div className="p-2 rounded-xl bg-sky-100 text-sm break-words">
                        ...
                        message?.type?.startsWith("image/") ? (
                          ...
                          <audio
                            className="w-[200px] h-[60px] md:w-[500px] "
                            controls
                          >
