...
const UserVerifyData = () => {
  ...
  const validateInput = (key: keyof FormDataType, value: string): boolean => {
    if (key === 'pincode') {
      return /^\d{6}$/.test(value); // Assuming pincode is a 6-digit number
    }
    return value.trim() !== '';
  };
  ...
  const handleChange = (key: keyof FormDataType) => (event: React.ChangeEvent<HTMLInputElement>) => {
    const value = event.target.value;
    setFormData((prevData) => ({ ...prevData, [key]: value }));
    setFormErrors((prevErrors) => ({ ...prevErrors, [key]: !validateInput(key, value) }));
  };

  const handleSubmit = async () => {
    const isValidForm = Object.entries(formData).every(([key, value]) => validateInput(key as keyof FormDataType, value));
                ...
                return (
                  <div key={key}>
                    <div className="flex justify-between px-2">
                      <span className="text-zinc-500 md:text-xl">{key.charAt(0).toUpperCase() + key.slice(1)}</span>
                      <div className="flex justify-between gap-1">
                        ...
                        <span className={`text-xs text-center ${error ? 'text-red-500' : 'text-green-500'}`}>
                          {formData[key as keyof FormDataType].length > 0 &&(error ? 'Invalid' : 'Valid')}
                        </span>
                      </div>
                    </div>
                    <input
                      type="text"
                      className={`w-[300px] border-2 px-4 py-3 rounded-lg lg:w-[400px] ${error ? 'border-red-500' : ''}`}
