...
const UserChatPage: React.FC = () => {
  ...
  useEffect(() => {
    const startConversation = async () => {
      try {
        const response = await axios.post('/user/start-conversation', {
          userId: currentUser?._id,
          adminId: currentUser?.adminId,
          propId: currentUser?.propId,
        });
    ...
    if (!currentUser.conversationId) {
      ...
      fetchMessages(currentUser.conversationId);
    }
  }, [currentUser]);
  ...
  const handleSignOut = () => {
    dispatch(signoutSuccess()); // Add sign out logic here
    navigate('/new_user'); // Redirect to QR scan page
  };
        ...
        <div className="flex-1 overflow-y-auto py-4">
          <div className="flex flex-col gap-4 px-5">
            {messages.map((message) => (
              <div key={message._id} className={`flex ${message.senderModel === "User" ? "justify-end" : "justify-start"} gap-3`}>
                ...
                <div className="flex flex-col">
                  <div className="flex justify-between text-xs text-slate-400">
                    ...
                    <span>{message.senderModel === "User" ? "You" : message.senderName}</span>
                  </div>
